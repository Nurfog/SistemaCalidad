...
const Layout = () => {
    ...
    const menuItems = [
        ...
        ...(user?.Rol?.includes('Administrador') || user?.Rol?.includes('Escritor') || user?.Rol?.includes('Responsable') ? [{ name: 'Redactor', path: '/redactar', icon: <FileText size={20} /> }] : []),
        ...(user?.Rol?.includes('Administrador') || user?.Rol?.includes('AuditorExterno') || user?.Rol?.includes('AuditorInterno') ? [{ name: 'Auditoría', path: '/auditoria', icon: <History size={20} /> }] : []),
        ...(user?.Rol?.includes('Administrador') ? [{ name: 'Usuarios', path: '/usuarios', icon: <User size={20} /> }] : []),
    ];
